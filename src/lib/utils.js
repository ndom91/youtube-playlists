export const parseYoutubeUrl = Url => {
  ...
  if (Url.includes('youtu.be')) {
    videoId = Url.substring(Url.indexOf('tu.be/') + 6)
  } else if (Url.includes('youtube.com')) {
    videoId = Url.substring(Url.indexOf('v=') + 2, Url.length).substring(0, 11)
...
export const addVideoToHash = videoId => {
  ...
  window.history.replaceState(
    null,
    null,
    `#${encodeURIComponent(window.btoa(x))}`
  )
}
...
export const removeVideoFromHash = videoId => {
  ...
  if (videoIdsFromUrl.includes(videoId)) {
    if (Array.isArray(videoIdsFromUrl)) {
      const remainingVideos = JSON.stringify(
        videoIdsFromUrl.filter(id => id !== videoId)
      )
      window.history.replaceState(
        null,
        null,
        `#${encodeURIComponent(window.btoa(remainingVideos))}`
      )
    } else {
...
export const fetchVideoDetails = async id => {
  return window
    .fetch(`https://yt-details.ndo.workers.dev/?vid=${id}`, {
      headers: {
